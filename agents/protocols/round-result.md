...
- progress: {claimed_kind, main_problem_effect, scope_limitations, evidence_level}.
  ...
  candidate-solution. evidence_level is at most proof-draft. The runner derives and seals the progress RESULT from this object,
  summary, active_gap, closed_gaps/new_gaps, evidence and next_target/acceptance.
  Do not write a second RESULT or seal it manually. PLAN still needs sealing
  before research; an independent REVIEW is never generated by the runner.
  Older results without progress remain readable via the legacy assessment path.
